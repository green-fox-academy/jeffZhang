...
class Updoot {
  ...
  addAlert(path, type, info) {
    ...
    const [ulEl] = $(`#${id}_response`).append(
      `<div class="alert alert-${type}">${id} / ${info}</div>`
    )
  ...
  async request(method, path, requestData = {}, validatorFn) {
    ...
    try {
      ...
      if (validatorFn && validatorFn(error.responseJSON || {}, error)) {
        return this.addAlert(
          `${endpointName}`,
          'success',
          `correct error response - ${JSON.stringify(error)}`
        )
      } else {
        return this.addAlert(
          `${endpointName}`,
          'danger',
          `unexpected error response - ${JSON.stringify(error)}`
        )
      }
    }
    ...
    if (validatorFn && !validatorFn(responseData)) {
      return this.addAlert(
        `${endpointName}`,
        'danger',
        `incorrect response - ${JSON.stringify(responseData)}`
      )
    } else {
      return this.addAlert(
        `${endpointName}`,
  ...
  async onGreeterClick() {
    ...
    await this.request(
      ...
      data => {
        return (
          data['welcome_message'] === 'Oh, hi there petike, my dear student!'
        )
      }
    )
  }
